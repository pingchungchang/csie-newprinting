...
def create_submission(username, printer, pages, money):
    """建立新任務，並取得 Ubuntu ID (uid)"""
    query = """
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(query, (username, printer, pages, money))
                uid = cur.fetchone()[0]
                conn.commit()
                logger.info(f"[DB] Created job UID: {uid} for user {username}")
                return uid
